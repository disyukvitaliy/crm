require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

RSpec.describe IssuesController, type: :controller do

  # This should return the minimal set of attributes required to create a valid
  # Issue. As you add validations to Issue, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    attributes_for(:issue)
  }

  let(:invalid_attributes) {
    attributes_for(:invalid_issue)
  }

  let(:valid_issue) {
    create(:issue)
  }

  let(:valid_project) {
    create(:project, creator: admin_user)
  }

  let(:valid_session) { {} }

  before(:all) { @project = create(:project, creator: admin_user)}
  after(:all)  { @project.delete}

  before(:each) { sign_in as_admin_user}

  describe 'GET #index' do
    it 'assigns all issues as @issues' do
      get :index, {project_id: @project.to_param}, valid_session
      expect(response).to have_http_status(:ok)
    end
  end

  describe 'GET #show' do
    it 'assigns the requested issue as @issue' do
      issue = valid_issue
      get :show, {id: issue.to_param}, valid_session
      expect(assigns(:model_object)).to eq(issue)
    end
  end

  describe 'GET #new' do
    it 'assigns a new issue as @issue' do
      get :new, {project_id: @project.to_param}, valid_session
      expect(assigns(:model_object)).to be_a_new(Issue)
    end
  end

  describe 'GET #edit' do
    it 'assigns the requested issue as @issue' do
      issue = valid_issue
      get :edit, {id: issue.to_param}, valid_session
      expect(assigns(:model_object)).to eq(issue)
    end
  end

  describe 'POST #create' do
    context 'with valid params' do
      it 'creates a new Issue' do
        expect {
          post :create, {project_id: @project.to_param, issue: valid_attributes}, valid_session
        }.to change(Issue, :count).by(1)
      end

      it 'assigns a newly created issue as @issue' do
        post :create, {project_id: @project.to_param, issue: valid_attributes}, valid_session
        expect(assigns(:model_object)).to be_a(Issue)
        expect(assigns(:model_object)).to be_persisted
      end

      it 'redirects to the created issue' do
        post :create, {project_id: @project.to_param, issue: valid_attributes}, valid_session
        expect(response).to redirect_to(action: :edit, id: Issue.last.to_param)
      end
    end

    context 'with invalid params' do
      it 'assigns a newly created but unsaved issue as @issue' do
        post :create, {project_id: @project.to_param, issue: invalid_attributes}, valid_session
        expect(assigns(:model_object)).to be_a_new(Issue)
      end

      it 're-renders the new template' do
        post :create, {project_id: @project.to_param, issue: invalid_attributes}, valid_session
        expect(response).to render_template(:new)
      end
    end
  end

  describe 'PUT #update' do
    context 'with valid params' do

      it 'updates the requested issue' do
        issue = valid_issue
        new_attributes = valid_attributes
        put :update, {id: issue.to_param, issue: {subj: :okokok}}, valid_session
        byebug
        issue.reload
        expect(Issue.find(issue.id).subj).to eq(:okokok)
      end

      it 'assigns the requested issue as @issue' do
        issue = valid_issue
        put :update, {id: issue.to_param, issue: valid_attributes}, valid_session
        expect(assigns(:issue)).to eq(issue)
      end

      it 'redirects to the issue' do
        issue = valid_issue
        put :update, {id: issue.to_param, issue: valid_attributes}, valid_session
        expect(response).to redirect_to(issue)
      end
    end

    context 'with invalid params' do
      it 'assigns the issue as @issue' do
        issue = Issue.create! valid_attributes
        put :update, {id: issue.to_param, issue: invalid_attributes}, valid_session
        expect(assigns(:issue)).to eq(issue)
      end

      it 're-renders the edit template' do
        issue = Issue.create! valid_attributes
        put :update, {id: issue.to_param, issue: invalid_attributes}, valid_session
        expect(response).to render_template('edit')
      end
    end
  end

  describe 'DELETE #destroy' do
    it 'destroys the requested issue' do
      issue = Issue.create! valid_attributes
      expect {
        delete :destroy, {id: issue.to_param}, valid_session
      }.to change(Issue, :count).by(-1)
    end

    it 'redirects to the issues list' do
      issue = Issue.create! valid_attributes
      delete :destroy, {id: issue.to_param}, valid_session
      expect(response).to redirect_to(issues_url)
    end
  end

end
